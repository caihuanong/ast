...
//pos? : // byte position of start of node in full original input string
...
// create a new numberNode from a text string
...
// create a new DurationNode from a text string
...
type UnaryNode struct {
	position
	Node     Node  		//one argument
	Operator TokenType	//an operator.
	Comment  *CommentNode
}
...
// 保存 InfluxDB database & retention policy 信息
...
//Holds the textual representation of an identifier
//属性和IdentifierNode几乎一样，两者有什么区别呢？ReferenceNode更像是一种引用，会有特殊字符
...
//以其他node作为参数的函数node，哪种node可以作为函数参数呢？
